    ...
    public static void main(String[] args) {
        ...
        try (Scanner sc = new Scanner(System.in)) {
            ...
            double nthHighestSalary = employees.stream()
                    ...
                    .orElseThrow(() -> new IllegalArgumentException("nth highest salary not found")); // Throw an
                                                                                                      // exception if n
                                                                                                      // is out of
                                                                                                      // bounds
            ...
            employees.stream()
                    ...
                    .ifPresent(e -> System.out
                            .println(n + "th highest salary: " + e.salary() + "\nemployee name = " + e.name()));

            NthHighestSalaryEmployee nthSalaryEmp = employees.stream()
                    .sorted((a, b) -> Double.compare(b.salary(), a.salary())).skip(n - 1).findFirst()
                    .orElseThrow(() -> new IllegalArgumentException("nth highest salary not found"));
            System.out.println(n + "th highest salary: " + nthSalaryEmp);

            // Alternatively, you can sort the employees by salary and then get the nth
            // highest salary
            ...
            System.out.println(n + "th highest salary: " + employees.get(n - 1).salary() + "\nemployee name = "
                    + employees.get(n - 1).name());

            employees.sort((c, d) -> Double.compare(d.salary(), c.salary()));
            System.out.println(n + "th highest salary: " + employees.get(n - 1).salary() + "\nemployee name = "
                    + employees.get(n - 1).name());
